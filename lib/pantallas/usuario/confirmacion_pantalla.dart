...
class ConfirmacionScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // ── Ruta rápida: la reserva recién creada está en el notifier ────────────
    // crearReserva() guarda la reserva con el ID correcto en el estado.
    // Esto evita un request extra a Firestore en el flujo normal.
    final notifierState = ref.watch(reservaNotifierProvider);
    final cachedReserva = notifierState.value;
    if (cachedReserva != null && cachedReserva.id == reservaId) {
      return Scaffold(
        backgroundColor: AppColors.bg,
        body: _ConfirmacionContenido(reserva: cachedReserva),
      );
    }

    // ── Fallback: deep link o acceso desde "mis reservas" ───────────────────
    // Usa FutureProvider con timeout de 8 s (nunca queda cargando infinito).
    final reservaAsync = ref.watch(reservaFutureProvider(reservaId));
    ...
    return Scaffold(
      ...
      body: reservaAsync.when(
        ...
        error: (e, _) => _ConfirmacionError(
          onRetry: () => ref.invalidate(reservaFutureProvider(reservaId)),
        ),
        data: (reserva) {
          if (reserva == null) {
            return _ConfirmacionError(
...
class _ConfirmacionError extends StatelessWidget {
  final String mensaje;
  final VoidCallback onRetry;

  const _ConfirmacionError({
    this.mensaje = 'No se pudo cargar la información de la reserva.',
